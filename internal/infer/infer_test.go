...
func TestInfer(t *testing.T) {
	...
	for _, test := range testCases {
		...
		td, results, err := Infer(constTree, geneTrees, InferOptions{runtime.GOMAXPROCS(0), *qopts})
		if err != nil {
...
func TestInfer_Large(t *testing.T) {
	...
	for _, test := range testCases {
		t.Run(test.name, func(t *testing.T) {
			...
			qopts, _ := pr.SetQuartetFilterOptions(0, 0)
			td, results, err := Infer(tre, quartets.Trees, InferOptions{runtime.GOMAXPROCS(0), *qopts})
			if err != nil {
...
func BenchmarkInfer(b *testing.B) {
	...
	for b.Loop() {
		qopts, _ := pr.SetQuartetFilterOptions(0, 0)
		_, _, err := Infer(tre, quartets.Trees, InferOptions{runtime.GOMAXPROCS(0), *qopts})
		if err != nil {
